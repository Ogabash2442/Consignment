...
export const LiveChatSystem: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedUserId && savedName && savedEmail) {
        setUserIdSession(savedUserId);
        setName(savedName);
        setEmail(savedEmail);
        setIsRegistered(true);
      }
    } catch (e) {
      console.warn("[LiveChatSystem] Failed to retrieve session from localStorage:", e);
    }
  }, []);
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    // STEP 1 & 2: Check localStorage for chat_user_id. If exists, reuse SAME ID.
    let userId = localStorage.getItem('chat_user_id');
    
    // STEP 3: If does not exist, generate new ID
    if (!userId) {
      let uuid = '';
      try {
      ...
      if (!uuid) {
        uuid = Math.random().toString(36).substring(2, 15) + '_' + Date.now().toString(36);
      }
      userId = 'client_' + uuid;
    ...
    // Seed welcoming intro text trigger - this also updates the state inside LogisticsContext
    sendUserMessage(userId, `System: Registered session for ${name.trim()}. Connecting with line officer.`, name.trim(), email.trim());
  };
