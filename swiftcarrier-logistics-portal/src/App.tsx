...
function AppContent() {
  ...
  // Dynamically update browser favicon if custom logo image is uploaded
  useEffect(() => {
    if (branding?.websiteLogo && (branding.websiteLogo.startsWith('data:') || branding.websiteLogo.startsWith('http'))) {
      const link: HTMLLinkElement | null = document.querySelector("link[rel*='icon']");
      if (link) {
        link.href = branding.websiteLogo;
      } else {
        const newLink = document.createElement('link');
        newLink.rel = 'icon';
        newLink.href = branding.websiteLogo;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-12 gap-12 pb-12 border-b border-white/5">
          ...
          <div className="md:col-span-4 space-y-4">
            <div className="flex items-center gap-2.5">
              {branding.footerLogo && (branding.footerLogo.startsWith('data:') || branding.footerLogo.startsWith('http')) ? (
                <div className="bg-white p-1 rounded-lg border border-slate-850 overflow-hidden flex items-center justify-center h-10 w-auto max-w-[150px]">
                  <img 
