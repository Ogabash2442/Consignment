...
export const LoginAccessDashboard: React.FC = () => {
  ...
  const handleLogoRemove = () => {
    const defaultURI = "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><rect width='100' height='100' rx='24' fill='%230f172a'/><path d='M50 20 L80 35 L80 65 L50 80 L20 65 L20 35 Z' fill='none' stroke='%2338bdf8' stroke-width='6' stroke-linejoin='round'/><path d='M50 20 L50 80 M20 35 L50 50 L80 35' fill='none' stroke='%2338bdf8' stroke-width='4' stroke-linejoin='round'/><circle cx='50' cy='50' r='10' fill='%230ea5e9' opacity='0.8'/></svg>";
    setBWebsiteLogo(defaultURI);
    setBHeaderLogo(defaultURI);
    setBFooterLogo(defaultURI);
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuthError('Authentication terminal rejected credentials: ' + (err.message || 'Please verify username and password.'));
    }
  };
  ...
  const handleUpdateCredentials = async (e: React.FormEvent) => {
    ...
    try {
      const { updateAdminCredentials } = await import('../admin');
      await updateAdminCredentials(newUsername, settingsPassword);
      ...
      try {
        ...
        console.warn("Express credential backup offline, using Firestore primary:", e);
      }
    } catch (err: any) {
      console.error('Failed to update operator credentials on Firestore:', err);
      setAuthError(err.message || 'Failed to publish credentials change on central node.');
    }
  };
                      ...
                      <form onSubmit={handleCreateOrEditShipment} className="space-y-5 text-xs">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            >
                              {['Processed', 'In Transit', 'Out for Delivery', 'Delivered', 'Pending', 'Exception', 'HOLD'].map((opt) => (
                        ...
                        <div key={s.trackingId} className="p-4 layout grid grid-cols-12 gap-2 text-xs items-center hover:bg-slate-50/50 select-none">
                          ...
                          <div className="col-span-12 mt-3 pt-3 border-t border-slate-50 grid grid-cols-1 md:grid-cols-12 gap-3 bg-slate-50 p-3 rounded-xl border">
                            ...
                            <div className="md:col-span-3 text-left">
                              ...
                              >
                                {['Processed', 'In Transit', 'Out for Delivery', 'Delivered', 'Exception', 'Pending', 'HOLD'].map((opt) => (
                                  <option key={opt} value={opt}>{opt}</option>
                                ))}
                              </select>
                            </div>
                      ...
                      <form onSubmit={handleUpdateBranding} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="border border-dashed border-slate-200 rounded-2xl p-4 bg-slate-50/50 space-y-3.5 transition-all">
                              <label className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Identity Logo Image</label>
                              ...
                              <div
                                ...
                                className={`h-28 rounded-xl border border-dashed flex flex-col items-center justify-center p-4 transition-all cursor-pointer text-center select-none ${
                                  logoDragOver ? 'border-sky-500 bg-sky-50/50' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50/30'
                                }`}
                              >
                                <input
                                  type="file"
                                  ...
                                  className="hidden"
                                />
                                <Upload className={`h-6 w-6 mb-2 transition-all ${logoDragOver ? 'text-sky-500 scale-110' : 'text-slate-400'}`} />
                                <span className="text-[11px] font-bold text-slate-700">Drag logo here or click to browse</span>
                                <span className="text-[9px] text-slate-400 font-light mt-0.5">PNG, JPG, JPEG, SVG or WEBP formats supported</span>
                              </div>

                              {/* Preview Area */}
                              {bWebsiteLogo && (bWebsiteLogo.startsWith('data:') || bWebsiteLogo.startsWith('http')) ? (
                                <div className="bg-white border rounded-xl p-3 flex items-center justify-between gap-4">
                                  <div className="flex items-center gap-3 overflow-hidden">
                                    <div className="h-12 w-12 bg-slate-50 border rounded-lg p-1.5 flex items-center justify-center overflow-hidden">
                                      <img src={bWebsiteLogo} alt="Preview" className="h-full w-full object-contain" />
                                    </div>
                                    <div className="text-left">
                                      <span className="text-[10px] font-bold text-slate-800 block truncate max-w-[140px]">custom_identity_logo</span>
                                      <span className="text-[9px] text-emerald-500 font-mono">Synchronized globally</span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex gap-2 shrink-0">
                                    ...
                                    <button
                                      ...
                                      className="text-[10px] font-bold bg-rose-50 hover:bg-rose-100 text-rose-600 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                                    >
                                      Reset Default
                                    </button>
                                  </div>
                                ...
                                <div className="text-center py-2 text-[10px] italic text-slate-400">
                                  No logo uploaded. Standard text fallbacks are active.
                                </div>
                              )}
                            </div>
                          </div>
