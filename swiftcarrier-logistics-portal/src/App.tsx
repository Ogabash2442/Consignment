...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between font-sans selection:bg-sky-500 selection:text-white">
      ...
      {activeAlert && (
        <div className={`px-4 py-2.5 text-xs font-medium text-left flex justify-center items-center gap-3 transition-all animate-fade-in ${
          activeAlert.category === 'delay' 
            ? 'bg-amber-500 text-slate-950 border-b border-amber-400' 
            : activeAlert.category === 'success'
              ? 'bg-emerald-600 text-white'
              : 'bg-slate-900 border-b border-slate-850 text-slate-100'
        }`}>
          <Megaphone className="h-4 w-4 flex-shrink-0 animate-bounce" />
          <span className="truncate leading-normal">
            <strong>System Notice:</strong> {activeAlert.title} — {activeAlert.content}
          </span>
          <span className="hidden md:inline font-mono text-[9px] opacity-75">{activeAlert.timestamp} UTC</span>
        </div>
      )}
