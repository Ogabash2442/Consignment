  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-100 bg-white/80 backdrop-blur-md transition-all duration-300">
      ...
      <div className="mx-auto flex max-w-7xl h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 border border-slate-200 overflow-hidden text-sky-400 font-bold font-mono text-sm shadow-md transition-transform group-hover:rotate-6 select-none">
            {branding.headerLogo && (branding.headerLogo.startsWith('data:') || branding.headerLogo.startsWith('http')) ? (
              ...
              <span className="text-slate-800 font-bold">{branding.headerLogo || "A"}</span>
            )}
            <div className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full border-2 border-white bg-sky-450 bg-sky-450 bg-sky-400"></div>
          </div>
          <div>
