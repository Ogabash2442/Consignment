...
export const LogisticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const setupChatSync = async () => {
      try {
        if (db) {
          console.log("Subscribing to Firestore 'chats' collection in real-time.");
          const chatsColRef = collection(db, "chats");
          
          unsubscribeChats = onSnapshot(chatsColRef, (snapshot) => {
            ...
            console.warn("Firestore listener failed. Switching to central backing store API route fallback.", err);
