...
export const isDemoMode = true; // Enforced true to build a lightweight, 100% stable full-stack container architecture without cloud Firebase dependencies
...
const activeChatFetchers = new Map<string, () => void>();
let allChatsFetcher: (() => void) | null = null;

export const triggerChatPoll = (chatId: string) => {
  if (typeof window === 'undefined') return;
  const fetcher = activeChatFetchers.get(chatId);
  if (fetcher) {
    fetcher();
  }
};

export const triggerAllChatsPoll = () => {
  if (typeof window === 'undefined') return;
  if (allChatsFetcher) {
    allChatsFetcher();
  }
  ...
  if (isDemoMode) {
    try {
      await fetch('/api/chats', {
        ...
        body: JSON.stringify({ chatId, userName, userEmail })
      });
      triggerAllChatsPoll();
    } catch (e) {
      console.error('Failed to sync started chat with container API', e);
    }
    return chatId;
  }
  ...
  if (isDemoMode) {
    const fetchFresh = async () => {
      try {
        ...
        console.error('Polling single chat error', err);
      }
    };
    activeChatFetchers.set(chatId, fetchFresh);
    fetchFresh();
    const interval = setInterval(fetchFresh, 1205);
    return () => {
      clearInterval(interval);
      activeChatFetchers.delete(chatId);
    };
  }
  ...
  if (isDemoMode) {
    const fetchFresh = async () => {
      try {
        ...
        console.error('Polling all chats error', err);
      }
    };
    allChatsFetcher = fetchFresh;
    fetchFresh();
    const interval = setInterval(fetchFresh, 1205);
    return () => {
      clearInterval(interval);
      if (allChatsFetcher === fetchFresh) {
        allChatsFetcher = null;
      }
    };
  }
  ...
  if (isDemoMode) {
    try {
      await fetch(`/api/chats/${chatId}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sender, message: messageText, attachment })
      });
      triggerChatPoll(chatId);
      triggerAllChatsPoll();
    } catch (e) {
      console.error('Failed to dispatch support message', e);
    }
    return;
  }
  ...
  if (isDemoMode) {
    try {
      await fetch(`/api/chats/${chatId}/typing`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sender, isTyping })
      });
      triggerChatPoll(chatId);
    } catch {
      // Ignored
...
export const deleteChatMessage = async (chatId: string, timestamp: string, adminEmail: string): Promise<void> => {
  if (isDemoMode) {
    try {
      await fetch(`/api/chats/${chatId}/messages/${encodeURIComponent(timestamp)}`, {
        method: 'DELETE'
      });
      // Trigger a local storage sync update for listeners inside client if any
      const data = localStorage.getItem('sc_chats');
      const chats: Chat[] = data ? JSON.parse(data) : [];
...
export const isUserEmailBlocked = async (email: string): Promise<boolean> => {
  ...
  if (isDemoMode) {
    try {
      const res = await fetch(`/api/blocked_users/${encodeURIComponent(cleanEmail)}`);
      if (res.ok) {
...
export const assignChatAdmin = async (chatId: string, adminEmail: string | null): Promise<void> => {
  if (isDemoMode) {
    try {
      await fetch(`/api/chats/${chatId}/assign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
// ====== GLOBAL BROADCAST / EXTRAS FOR COURIER ANNOUNCEMENTS AND ALERTS ======
...
export const getAnnouncements = async (): Promise<Announcement[]> => {
  ...
  try {
    const snaps = await getDocs(query(collection(db, path), orderBy('timestamp', 'desc')));
    return snaps.docs.map(doc => ({ id: doc.id, ...doc.data() } as Announcement));
  } catch {
    return [];
  }
};

export const createAnnouncement = async (title: string, body: string, author: string): Promise<void> => {
  if (isDemoMode) {
    try {
  ...
  try {
    await setDoc(doc(db, path, id), { title, body, author, timestamp: new Date().toISOString() });
  } catch (error) {
    handleFirestoreError(error, OperationType.CREATE, `${path}/${id}`);
  }
};
...
export const getNotifications = async (): Promise<AdminNotification[]> => {
  ...
  try {
    const snaps = await getDocs(query(collection(db, path), orderBy('timestamp', 'desc')));
    return snaps.docs.map(doc => ({ id: doc.id, ...doc.data() } as AdminNotification));
  } catch {
    return [];
  }
};
...
export const clearNotifications = async (): Promise<void> => {
  ...
  try {
    ...
    snaps.forEach(async (d) => {
      await updateDoc(doc(db, path, d.id), { read: true });
    });
  } catch (error) {
