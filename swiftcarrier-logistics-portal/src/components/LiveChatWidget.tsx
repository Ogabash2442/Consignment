  ...
  useEffect(() => {
    ...
    const unsubscribe = listenToChat(chatId, (updatedChat) => {
      if (updatedChat) {
        ...
        // Safe reconnect and race-condition guard: do not delete the local storage keys automatically 
        // when experiencing transient null responses or server startup latency.
        setChat(null);
      }
    });
        ...
        >
          ...
          <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-900 leading-normal text-xs relative flex flex-col justify-start">
            ...
            {needsReg ? (
              ...
              <div className="my-auto flex flex-col items-center justify-center text-slate-400 space-y-3 font-mono w-full text-center">
                <Loader2 className="h-6 w-6 animate-spin text-[#ff3c00]" />
                <span className="text-[10px] uppercase tracking-wider">Establishing Connection Line...</span>
              </div>
            ) : (
              /* STAGE B: Conversational Messaging Workspace */
              <div className="space-y-3.5 flex-1 flex flex-col justify-start overflow-y-auto">
