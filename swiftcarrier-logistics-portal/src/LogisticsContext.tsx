...
export const LogisticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addShipment = (shipment: Shipment) => {
    setShipments((prev) => [JSON.parse(JSON.stringify(shipment)) as Shipment, ...prev]);
  };

  const updateShipment = (updatedShipment: Shipment) => {
    setShipments((prev) => {
      return prev.map((s) => {
        if (s.trackingId.toUpperCase().trim() === updatedShipment.trackingId.toUpperCase().trim()) {
          // Perform deep clone to completely shield against reference pollution or shared object mutation
          return JSON.parse(JSON.stringify(updatedShipment)) as Shipment;
        }
        return s;
      });
    });
