...
export const LogisticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Synchronized Firebase Auth persistence & real-time chat/subcollection listeners
  useEffect(() => {
    let unsubscribeAuth: (() => void) | null = null;
    let unsubscribeChats: (() => void) | null = null;
    const subcollectionUnsubs: { [userId: string]: () => void } = {};
    ...
    if (auth) {
      ...
      unsubscribeAuth = onAuthStateChanged(auth, (user) => {
        ...
        // Check if user is authenticated AND admin flag is true
        if (user && adminFlag) {
          console.log("Admin persistent session verified: UID =", user.uid);
          setIsAuthenticated(true);
          setIsAdminLoading(false);

          // Real-time listen to chats collection ONLY after authentication is verified
          if (db) {
            console.log("Active Admin: Starting real-time Firestore synchronization for chats + subcollections.");
            ...
            unsubscribeChats = onSnapshot(chatsColRef, (chatsSnapshot) => {
              ...
              chatsSnapshot.docs.forEach((chatsDoc) => {
                ...
                chatHeaderList.push({
                  ...
                  messages: data.messages || [] // Start with existing field messages
                });
              });

              // Clean up unused message subcollection listeners for chats that no longer exist
              Object.keys(subcollectionUnsubs).forEach((id) => {
                if (!activeIds.has(id)) {
                  subcollectionUnsubs[id]();
                  delete subcollectionUnsubs[id];
                }
              });

              // Now set up real-time listener for messages subcollections of each active chat session
              chatHeaderList.forEach((chat) => {
                const uId = chat.userId;
                if (!subcollectionUnsubs[uId]) {
                  const submsgsColRef = collection(db!, "chats", uId, "messages");
                  subcollectionUnsubs[uId] = onSnapshot(submsgsColRef, (submsgsSnapshot) => {
                    const messagesList: ChatMessage[] = [];
                    submsgsSnapshot.docs.forEach((msgDoc) => {
                    ...
                    // If nested array exists and matches subcollection, or subcollection is non-empty, merge/update
                    setChats((prevChats) => {
                      return prevChats.map((c) => {
                        if (c.userId === uId) {
                          return {
                            ...c,
                            messages: messagesList.length > 0 ? messagesList : c.messages
                          };
                        }
                        return c;
                      });
              ...
              setChats(chatHeaderList);
            }, (error) => {
              console.warn("Firestore admin chats listen failed, fall back.", error);
          ...
          // Clear any active chat collection subcollection listener we have
          if (unsubscribeChats) {
            unsubscribeChats();
          ...
          // Run polling fallback for non-admin viewers/agents
          startPollingFallback();
        }
      });
    } else {
      // Offline fallback
      console.warn("Firebase Auth is offline or disabled. Falling back to HTTP polling.");
  ...
  // Real-time synchronization for customer-side (non-admin) viewers to receive admin replies instantly
  useEffect(() => {
    if (!db || isAuthenticated) return; // Admin is already handled by the top-level auth wrapper

    // Identify chats that are initiated by clients in this browser
    const clientChats = chats.filter(c => c.userId.startsWith('client_'));
    const unsubs: (() => void)[] = [];

    clientChats.forEach(chat => {
      const chatDocRef = doc(db!, "chats", chat.userId);
      const submsgsColRef = collection(db!, "chats", chat.userId, "messages");

      // Real-time listener for main document
      ...
      // Real-time listener for message subcollection
