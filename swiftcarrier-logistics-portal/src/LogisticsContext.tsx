...
export const LogisticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Real-time synchronization for live chats
  useEffect(() => {
    const fetchChats = async () => {
      try {
        ...
        // Silent catch during server reboots
      }
    };
    
    fetchChats();
    const interval = setInterval(fetchChats, 1200);
  ...
  const sendUserMessage = async (userId: string, text: string, userName?: string, userEmail?: string) => {
    ...
    try {
      ...
      console.error('Failed to send user message to server:', err);
    }
  };
  ...
  const sendAgentResponse = async (userId: string, text: string) => {
    ...
    try {
      ...
      console.error('Failed to send agent response to server:', err);
    }
  };
  ...
  const markChatResolved = async (userId: string) => {
    ...
    try {
      ...
      console.error('Failed to mark chat as resolved on server:', err);
    }
  };
