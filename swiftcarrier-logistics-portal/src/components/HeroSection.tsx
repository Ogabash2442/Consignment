    ...
    <div className="relative overflow-hidden bg-slate-950 py-16 sm:py-24 lg:py-28 font-sans">
        ...
        {SLIDES.map((slide, i) => (
          <img
            ...
            className={`absolute inset-0 h-full w-full object-cover object-center transition-all duration-[4000ms] ease-out ${
              i === currentSlide ? 'opacity-60 scale-100' : 'opacity-0 scale-105 pointer-events-none'
            } transform`}
          />
        ))}
        {/* Soft dual-layer dark cinematic overlay for premium high readability of texts with improved image clarity */}
        <div className="absolute inset-0 bg-slate-950/15" />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950/85 via-slate-950/70 to-slate-950/15" />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/60 via-transparent to-slate-950/5" />
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div key={currentSlide} className="lg:col-span-7 flex flex-col items-start gap-4 text-left animate-fade-in-up">
          {currentSlide === 0 && branding.websiteLogo && (branding.websiteLogo.startsWith('data:') || branding.websiteLogo.startsWith('http')) && (
            <div className="bg-white/90 p-2 rounded-xl border border-white/20 shadow-lg mb-2 max-h-16 flex items-center justify-center">
              <img 
                src={branding.websiteLogo} 
                alt={branding.siteName} 
                referrerPolicy="no-referrer"
                className="max-h-12 max-w-[180px] object-contain h-auto w-auto"
              />
            </div>
          )}
          {/* Tagline */}
