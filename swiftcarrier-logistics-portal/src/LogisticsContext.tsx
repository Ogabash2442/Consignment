...
export const LogisticsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Save changes to localStorage for local persistent behavior
  useEffect(() => {
  ...
  const sendUserMessage = (userId: string, text: string, userName?: string, userEmail?: string) => {
    setChats((prev) => {
    ...
    // Simulate standard fast operator response to look extremely alive and elegant!
    setTimeout(() => {
      setChats((prev) => {
        const chat = prev.find((c) => c.userId === userId);
        if (!chat) return prev;

        // If last message in chat is indeed from user, create matching automatic helpful reaction
        const lastMsg = chat.messages[chat.messages.length - 1];
        if (lastMsg && lastMsg.sender === 'user') {
          const nowStr = new Date().toISOString();
          const timestampStr = nowStr.slice(0, 10) + ' ' + nowStr.slice(11, 16);
          const autoKey = 'm_auto_' + Math.random().toString(36).substring(5);

          let replyText = "Thank you for contacting Apex Global Support. Our dispatcher has been notified of your message and will review your shipment details immediately. Is there a specific tracking ID you are inquiring about?";
          
          const textLower = text.toLowerCase();
          if (textLower.includes('apx-8820') || textLower.includes('watch') || textLower.includes('switzerland')) {
            replyText = "Inquiring about shipment APX-8820-CH? It is currently onboard transatlantic Flight APX-009, with stable telemetry inside the secure thermal vault. Biometric protocols are on-schedule for arrival at SEA.";
          } else if (textLower.includes('apx-4412') || textLower.includes('denim') || textLower.includes('fabric')) {
            replyText = "Inquiring about shipment APX-4412-JP? The double waterproof barrier is intact, with humidity sensors recording optimal values (no water intrusion). Final delivery is scheduled for Paris CDG zone shortly.";
          } else if (textLower.includes('deliver') || textLower.includes('late') || textLower.includes('when')) {
            replyText = "All active shipments are tracking on-schedule. For specific cargo arrival estimated hours, please type your direct tracking ID or check our dashboard stats section.";
          }
          ...
          return prev.map((c) => {
            if (c.userId === userId) {
              return {
                ...c,
                messages: [...c.messages, automaticMessage],
                status: 'active', // Mark it active since answered
                lastUpdated: nowStr
              };
            }
            return c;
    ...
    }, 4500); // 4.5 second delay so it feels highly premium and realistic!
  };

  const sendAgentResponse = (userId: string, text: string) => {
    setChats((prev) => {
      const nowStr = new Date().toISOString();
      const timestampStr = nowStr.slice(0, 10) + ' ' + nowStr.slice(11, 16);
  ...
  const markChatResolved = (userId: string) => {
    setChats((prev) =>
