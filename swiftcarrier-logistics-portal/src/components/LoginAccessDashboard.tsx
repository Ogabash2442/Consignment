...
export const LoginAccessDashboard: React.FC = () => {
  ...
  const [storedUsername, setStoredUsername] = useState(() => localStorage.getItem('apx_dashboard_username') || 'dispatcher_108');
  const [storedPassword, setStoredPassword] = useState(() => localStorage.getItem('apx_dashboard_password') || 'apex2026');
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (normalInputUser === normalStoredUser && loginPassword === storedPassword) {
      setIsAuthenticated(true);
      setAuthError('');
    } else if (normalInputUser === 'dispatcher' && loginPassword === 'apex2026' && storedUsername === 'dispatcher_108') {
      // Allow fallback default in all contexts
      setIsAuthenticated(true);
      setAuthError('');
    } else {
  ...
  const handleUpdateCredentials = (e: React.FormEvent) => {
    e.preventDefault();
    if (!settingsUsername.trim() || !settingsPassword) return;

    localStorage.setItem('apx_dashboard_username', settingsUsername.trim());
    localStorage.setItem('apx_dashboard_password', settingsPassword);
    setStoredUsername(settingsUsername.trim());
